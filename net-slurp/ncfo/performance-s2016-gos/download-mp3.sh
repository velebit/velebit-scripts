...
wget \
    ...
    http://www.familyopera.org/drupal/sites/default/files/2016_Giants_of_Science/16GoS-Performance/GoS16-17P_NeilDeGrasseTyson.mp3 \
  2>&1 | tee "$log"
if [ -f "$dir/$file" ]; then
    rm -f "$dir"/index.html "$dir/$file".orig
    mv "$dir/$file" "$dir"/index.html
    ./clean-up.pl -i "$file" -i index.html -i index.html.tmp "$log"
